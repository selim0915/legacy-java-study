...
public List<String> selectJobList(){
	...
	/*
	 * public Map<Integer, String> selectDeptList(){ Map<Integer, String> deptMap =
	 * new HashMap<Integer, String>();
	 * 
	 * Connection conn = null; PreparedStatement pstmt = null; ResultSet rs = null;
	 * 
	 * try { //POOL 연결 객체 얻어오기 String sql = "SELECT DEPTNO, DNAME FROM DEPT"; conn =
	 * ds.getConnection(); pstmt = conn.prepareStatement(sql); rs =
	 * pstmt.executeQuery();
	 * 
	 * if (rs.next()) { do { int deptno = rs.getInt("deptno"); String dname =
	 * rs.getString("dname"); deptMap.put(deptno, dname);
	 * 
	 * } while (rs.next()); } else { System.out.println("데이터가 없습니다."); }
	 * 
	 * }catch (Exception e){ e.getStackTrace(); }finally {
	 * 
	 * if(pstmt!=null)try {pstmt.close();}catch(Exception e) {} if(rs!=null)try
	 * {rs.close();}catch(Exception e) {} if(conn!=null)try
