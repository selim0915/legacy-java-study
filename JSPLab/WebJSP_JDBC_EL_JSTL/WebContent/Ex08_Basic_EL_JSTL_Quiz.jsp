...
<body>
	...
	checkbox 다중 선택
	hint) request.getParameterValue("subject"); >> 표현을 EL
...
<%-- 	<c:choose>
		<c:when test="${not empty paramValues.subject }">
 			...
 			<ul>
				<c:forEach var="subject" items="${ paramValues.subject }"
					varStatus="status">
					<li>${subject.index}- ${subject } - ${status.count }</li>
				</c:forEach>
			</ul>
		</c:when>
		<c:otherwise>
	...
	<select id="newYear" name="newYear" title="년도선택">
		...
		<option value="2019" <%if (year == 2019) {
				out.print("selected");
			}%>>2019</option>
	</select>
	<hr>
	...
	<select id="newYear" name="newYear" title="년도선택">
		...
		<option value="2017"
			<c:if test="${param.year == 2017}">selected</c:if>>2017</option>
		<option value="2018"
			<c:if test="${param.year == 2018}">selected</c:if>>2018</option>
		<option value="2019"
			<c:if test="${param.year == 2019}">selected</c:if>>2019</option>
	</select>
