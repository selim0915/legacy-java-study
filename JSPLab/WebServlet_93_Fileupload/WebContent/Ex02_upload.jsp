...
<!--
  ...
  2. 이클립스 톰켓 내장 : 실 배포 경로
    실제 파일 저장 경로(아래 있는 경로)
     C:\FrontEndBackEnd\WebJSP\JSPLab\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\WebServlet_93_Fileupload\upload
-->
...
<%
	...
	//cos.jar 파일에서 제공하는 MultipartRequest 객체사용
	...
	MultipartRequest multi = new MultipartRequest(
				request, //기존에 있는 request객체의 주소값
				uploadpath, //실저장경로(배포경로)
				size, //10m
				"UTF-8",
				new DefaultFileRenamePolicy() //파일중복(upload폴더 안에:a.jpg -> a_1.jpg (업로드변경))
			); //파일 업로드 완료
			
			
	//여기까지 수행하는 upload 폴더에 저장됨
	//자료실
	//DB insert 실제 원하는 작업
	//작성자, 제목, 내용, 조회수, [파일명], 추가적으로 [파일사이즈]
	...
	//	System,out.println(filename.nextElement()); //filename1, filename2
	//}
