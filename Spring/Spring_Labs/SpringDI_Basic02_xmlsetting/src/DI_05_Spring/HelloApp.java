...
//		System.out.println("single2: " + single2);
		...
		//KEY POINT
		//컨테이너 구성되고 xml read해서 객체생성 조립
		//필요하다면 컨테이너안에 있는 객체를 사용
		//getbean() return type Object >> down casting 사용
		
		//컨테이너 모든 객체의 타입은 무엇일까? (default : singleton)
		//getbean()이라는 함수는 호출할 떄마다 새로운 객체를 만들지 않는다.
		
		
		ApplicationContext context = 
				new GenericXmlApplicationContext("classpath:DI_05_Spring/DI_05.xml"); //classpath
		...
		System.out.println("after : mybean(name) 객체" + mybean3);
		...
		System.out.println("singleton2 객체 : " + single2);
	}
}
