...
--values 이하절에 select 절을 쓸수 있다.
--insert into 테이블명(컬럼리스트) select 구문 .. (단 컬럼의 개수와 타입이 동일해야한다)

insert into temp5(num) select id from temp4; -- 단 컬럼개수와 타입 동일 
select * from temp5;

--2. insert
--테이블이 없는 상황에서 [테이블생성]+[대량데이터삽입]
--단 복사개념 (제약정보는 복사 안되요 (pk, fk)
--순수한 구조정보 (컬럼이름, 타입) 복사

--create table 테이블명 (컬럼명 타입정보)
create table copyemp as select * from emp;
select * from copyemp;
--as select *from 테이블 :  구조(스키마)+데이터 까지 복제 insert한다.

create table copyemp2 as select empno, ename, sal from emp where deptno=30;
select * from copyemp2;

--질문) 구조만 복사하고 데이터는 복사하고 싶지 않을때,,?
create table copyemp3 as select * from emp where 1=2; --1=2 거짓일때
select * from copyemp3;
desc copyemp3;

--tip
--오라클 시스템 테이블 (다양한 정보)
--제약정보 (pk, fk..)가 걸려있는지 확인해야한다.
select * from user_constraints where table_name='EMP'; --데이터는 모두 대문자로 관리한다.
...
---------------------------------------------------------------------------------
--[delete]
--원칙: delete > commit > 복원불가 > 백업데이터가 있다면 가능
delete from copyemp;
select * from copyemp;
rollback;
...
/*
 ...
 Create : insert
 Read : select (전체조회, 조건조회)
 Update : update
 Dellte : delete
 
 (DML: create, update, delete) 트랜잭션은 commit, rollback을 강제해야한다.
...
--컬럼의 정보 보기
select * from user_tab_columns where table_name='VTABLE'; --DATA_DEFAULT 값이 어떤지 확인
...
--기존 테이블에 있는 컬럼이름 바꾸기 (rename)
alter table temp6 rename column ename to username;
desc temp6;

--4. 기존테이블에 기존 컬럼타입을 잘못 잡은 경우
--기존테이블에 타입정보 수정하기 (modify)
alter table temp6 modify(username varchar2(2000));
desc temp6;

--5. 기존테이블에 있는 기존 컬럼 삭제하기
alter table temp6 drop column username;
desc temp6;

--6. 기존테이블을 삭제하고 싶다.
--6.1 데이터만 삭제: delete
--테이블 처음 만들면 처음 크기를 부여 >> 데이터를 넣을수록 >> 데이터크기 증가 
--ex) 처음 1m >> 10만건 >> 100m >> delete10만건 삭제 >> 여전이 테이블의 크기는 100m
--??? 테이블 데이터 삭제하면 공간의 크기가 줄여지게 못하나여?
-->> truncate (where절 못씀), 공간은 남아있다. 테이블만 남게..
--ex) 처음 1m >> 10만건 >> 100m >> delete10만건 삭제 >> 여전이 테이블의 크기는 1m

--7. 테이블삭제 (drop)
--데이터도 지우고, 공간도 지우고, 테이블도 지운다.
drop table temp6;

select * from user_constraints where table_name='EMP'; --오라킁 not null제약을 본다
--**
create table temp7(
  --id number primary key --권장하지 않음(줄임표현) SYS_Coo7oo9 제약이름을 맘대로 만듬
...
--1. primary key는 테이블에 몇개까지 걸수 있나요? 
--A: 1개 (여러개를 묶어서 1개는 가능)
...
--그리고 select 결과는 학번 , 이름  총점, 평균 , 학과코드 , 학과명 을 출력하세요
...
desc major;
insert into major (mid, mname) values (100, '경영과');
insert into major (mid, mname) values (200, '수학과');
insert into major (mid, mname) values (300, '화학과');
select * from major;
commit;
...
insert into student (sid, name, kor, eng, mat, mid) values (1, '우세림', 100, 100, 98, 100);
insert into student (sid, name, kor, eng, mat, mid) values (2, '김동민', 60, 30, 10, 200);
insert into student (sid, name, kor, eng, mat, mid) values (3, '신지혁', 80, 90, 88, 300);
select * from student;
commit;
...
---View  오라클 192.pdf
--view객체 ... (가상테이블)
--create view 뷰이름 as [select구문](view를 통해서 볼 수 있는 데이터목록)
--view는 테이블처럼 사용가능 (가상테이블) -> emp, dept 같은 논리로 접근 사용
--view 메모리상에만 존재하는 가상테이블 (subquery -> in line view -> from (select deptno from emp..))

--view가상테이블
--사용법은 테이블사용법과 동일하다. (select .. where .. )
--dml: insert, update, delete -> view를 통해서 dml가능 -> view가 가지고있는 (볼 수 있는) 실 테이블데이터를 변경
...
as
  select empno, ename from emp;
select* from v_001; --view가 가지도 있는 sql문장을 실행한다.

--view사용목적
--1.개발자의 편리성 (join, subquert) 가상테이블개념
--2.편리성 (쿼리의 단순화) -> 복잡한 쿼리를 미리 만들어 두고 사용.
--3. 보안성 (권한처리를 통해서 노출하고 싶은 데이터만 모아서 view를 만든다. -> view권한을 부여
create view v_emp
as
...
create view v_003 --view도 가상테이블이다 (자존심: 나 컬럼명 만들어줘->조작된 컬럼에 컬럼명을 만들지 않으면 안된다.)
as
...
select * 
from emp e join v_003 s --저번엔 from (..) 인라인조인 사용
...
--view를 통한 dml작업이 가능합니까 (insert, update, delete)
--단, view를 통해서 볼수 있는 데이터에 관해서만
--복합 view (1개 이상의 테이블 구성) -> dml작업 불가라고 생각
--단순 view (테이블 1개로 만든 view 


select * from v_emp; --view통해서 볼 수 있는 데이터 (empno, ename, job, hiredate)
update v_emp set sal=0; --view를 통해서 sal데이터를 볼수 없다. (xxx)
update v_emp set job='IT'; --원본테이블에 가서 업데이트 변경해버린것..
select * from emp; --원본데이터가 바뀌어 버린다...!
rollback;
...
update v_emp2 set sal=0; --가지고 있는 데이터 중 가지가 볼수있는 데이터만 바꿔준다.
select * from v_emp2;
select * from emp;
rollback;

select * from user_tables; --테이블목록보기
select * from user_views; --뷰 목록 보기


------------------------------
--1. 30번 부서 사원들의 직위, 이름, 월급을 담는 view를 만든러아
create view v_emp3
as
  select job, ename, sal from emp where deptno=30;
select * from v_emp3;

--2. 30번 부서 사원들의  직위, 이름, 월급을 담는 VIEW를 만드는데, 각각의 컬럼명을 직위, 사원이름, 월급으로 ALIAS를 주고 월급이 300보다 많은 사원들만 추출하도록 하라.
-- create or replace view view001 (컬럼명, 컬럼명, .....)  
create or replace view v_emp4
as
  select job as "직위", ename as "사원이름", sal as "월급" from emp where sal > 300;
select *from v_emp4;

create or replace view v_emp8 (직위, 사원이름, 월급)
as
...
as
  select avg(sal) as "부서별 평균 월급" from emp group by deptno having avg(sal)>2000;
select * from v_emp6;
...
as
  select sum(sal) as "직위별 총 월급" from emp where job != 'MANAGER' group by job having sum(sal)>3000;
select * from v_emp7;
...
--boardid칼럼 (not null, unique, index 구성된다)
--개발자: boardid데이터는 
--개발자: where boardid =? 무조건 1개는 return

--insert into board(boardid, title) values (..
--그러면 중복되지 않고, null값이 아니다 라는 보장
...
*/
--조건: 처음 글을 쓰면 1이라는 값을 insert., 그다음부터 2ㅡ3ㅡ4 순차적인 값을 insert
...
insert into board(boardid,title) values ((select count(boardid) +1 from board), '제목2');
select * from board;

delete from board where boardid=1; --1번글 지워지면 중복됨
insert into board(boardid,title) values ((select count(boardid) +1 from board), '제목3');
rollback;

insert into board(boardid, title) values ((select max(boardid)+1 from board), '처음');
insert into board(boardid, title) values ((select max(boardid)+1 from board), '처음2');
insert into board(boardid, title) values ((select max(boardid)+1 from board), '처음3');
select * from board;

delete from board where boardid=1; --1번글 지워져도 중복안됨, 새로운 인덱스 창조 불가
select * from board;
insert into board(boardid, title) values ((select max(boardid)+1 from board), '처음4');


----------------------------
--sequence 번호 추출(규칙) : 중복값이 없고 순차적인 데이터를 만들 수 있다.
----------------------------
...
--몇번까지 햇는지 헷갈릴 때, 채번하지 않고 정보를 알려준다
select board_num.currval from dual; --현재까지의 정보를 알려준다.
...
insert into kboard(num, title) values (kboard_num.nextval, '두번째글');
insert into kboard(num, title) values (kboard_num.nextval, '세번째글');
select * from kboard;
select kboard_num.currval from dual; --현재까지 번호 알려듐

--------------------
--1. sequence 객체하나 (테이블간 공유객체) >> 여러개의 테이블에서 사용 가능(가능하다)
--A(1)(4), B(2), C(3)
...
--insert into board(title) values('방가') --> boardnum 1값이 자동 insert
--ms-sql 2012버전부터(sequence)
--my-sql:  create table board(boardnum int auto_increment, titlt varchar2(20))

-------------------------------------------
--sequence 객체 옵션
create sequence seq_num
start with 10 --시작이 10
increment by 2; --2씩 올라감
...
-----------------------------------------------------
--개발자 필수 2
--rownum 의사컬럼: 실제 물리적으로 존재하는 컬럼이 아니고 논리적으로 존재하는 컬럼 select한 결과를 기반으로
--rownum: 실제로 테이블에 컬럼으로 존재하진 않지만, 내부적으로 행에 번호를 부여할 수 있는 가상컬럼
--rowid: 주소값 (행이 실제로 저장되는 내부 주소값) -> 인텍스 만들 때 사용

select * from emp;
select rownum as "순번", empno, ename from emp; --select한 결과를 가지고 순번을 붙여준다.

--Top-n 쿼리
--테이블에서 조건에 맞는 상위(TOP)레코드 (row) 추출
--MS-SQL 서버
...
-----------------------------------
--rownum 사용
--1. 정렬의 기준을 정의하는 작업이 먼저 선행되어야 한다.
--2. 정렬이 된 상황에서 앞에 순번을 붙인다.
--3. 원하는 조건절을 제시하여 데이터를 추출한다.
...
--10건씩 나누어서 보여주기
--총100건
--pagesize =10 한화면에 페이지 보여줄 데이터 건수 row수
--page개수 >> 10
--1~10 1page (DB에서 1~10까지 select 가지고 앱 출력), 11~20 2page
--91~100 10page 마지막..
...
--41~50번째 사원데이터 출력 (조건 사번이 낮은 순)
--pagesize를 10으로 잡았다는 가정
...
from (
  select rownum as num, e.*
  from (
    select * from employees order by employee_id asc  --기준점
  ) e where rownum <=50
) s where num > 40;
...
--또는 between A and B >> where rownum between 10 and 20 사용됨
--------------------
--------------------
--3차 프로젝트전에
--변수, 제어문, 커서, 함수, 프로시져, 트리거, 스케줄 사용
...
--입사일을 출력하라. 입사일을 기준으로 해서 오름차순으로 정렬하라.
 select ename, job, to_char(hiredate,'yyyy-MM-dd') as "날짜" 
 from emp 
...
select upper(substr(ename,1,1))||lower(substr(ename,2)) as "이름", length(ename) as "이름길이" from emp;
...
select m.empno  as "관리자",  min(e.sal) as "최저급여" 
from emp e, emp m 
...
--커미션이 책정되지 않은 사원은 NoCommission 으로 표시하고, 커미션의 컬럼명은 Comm으로 나오게 출력하시오.
--    단, 최고월급부터 출력되게 하시오
select deptno from dept where loc='DALLAS';
